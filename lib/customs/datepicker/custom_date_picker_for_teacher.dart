...
class CustomDatePickerForTeacher extends StatefulWidget {
  final String hintLabel;
  final String textOnTopOfDatePicker;
  final Function(String result) callbackFromCustomDatePicker;

  const CustomDatePickerForTeacher(
      {Key? key, required this.hintLabel, required this.callbackFromCustomDatePicker, required this.textOnTopOfDatePicker})
      : super(key: key);

  @override
  State<CustomDatePickerForTeacher> createState() => CustomDatePickerForTeacherState();
}

class CustomDatePickerForTeacherState extends State<CustomDatePickerForTeacher> {
  ...
  Widget build(BuildContext context) {
    ...
    Positioned(top:0, left:15, child: Text(widget.textOnTopOfDatePicker,style: TextStyle(color: appBarforegroundColor),))
    ],);
  }
}
