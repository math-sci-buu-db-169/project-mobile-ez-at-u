...
class ItemActivityForTeacherRole extends StatelessWidget {
  final dynamic data;
  final dynamic title;
  final GestureTapCallback? onTap;
  const ItemActivityForTeacherRole({Key? key, required this.data, this.onTap, this.title }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    String name = data?.stdname??'-';
    String lastname = data?.stdlastname??'-';
    String sdate = data?.startdate??'-';
    String fdate = data?.finishdate??'-';
        ...
        children: [
          Container(
            ...
            child: Table(
              ...
              children: [
                _buildListActivity(
                    title: title?.textactivity?? "Activity name", dataDetail:data?.acname ??'-'),
                _buildListEmpty(),
